...
//   try {
//     setLoading(true); 
//     // 1. Removed authentication requirement here so anyone can register
//     const cvText = await extractTextFromFile(selectedFile);
//     // 2. Pass null or a guest identifier instead of user.id if your service function requires an ID
//     await saveCandidate(
//       {
//         basicData: basicData!,
//         professionalData: {
...
//         cvText,
//       },
//       'guest-user-id' // Or handle it accordingly inside your saveCandidate service
//     );
//     onNext(); // Proceed to success/dashboard step
//   } catch (error: any) {
//     console.error(error);
//     alert(`Submission failed: ${error.message}`);
//   } finally {
...
import { supabase } from '../supabaseClient';
import type { FormComponentProps } from '../types/candidate';
import { saveCandidate } from './candidateService';   // 👈 correct path
import { extractTextFromFile } from '../utils/extractCvText';
...
export default function UploadCV({
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      // 3. Save candidate — SAME function & SAME bucket for both cases
      await saveCandidate(
        {
          basicData: basicData!,
          professionalData: {
        ...
        user?.id ?? null // null for guests
      );

      // 4. Redirect based on auth state
      if (user) {
        onNext(); // Authenticated → Dashboard
      } else {
        window.location.href = '/'; // Guest → Home page
      }
    } catch (error: any) {
      console.error(error);
      alert(`Submission failed: ${error.message}`);
      ...
      <div className="w-full max-w-6xl mb-6">
        <div className="px-12 pt-4 pb-2">
          <div className="flex flex-wrap items-center gap-y-3 gap-x-5 text-xs font-medium text-slate-500">
            ...
            <div className="flex items-center gap-2">
              <div className="w-7 h-7 rounded-full bg-lime-500 text-white flex items-center justify-center text-sm font-bold shadow-sm">
                5
              </div>
